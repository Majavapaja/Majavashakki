...
describe("Check", () => {
  ...
  describe("King in check", () => {
    it("should not allow king to capture checking piece if it causes check", done => {
      const promise = factory.build("board-king-in-check").then(board => moveSequence(board, [["d1", "c1"]]))
      promise.should.eventually.have.same.members(["error"]).notify(done)
    })

    it("should not allow other pieces to move if king is in check", done => {
      const promise = factory.build("board-king-in-check").then(board => moveSequence(board, [["a1", "a8"]]))
      promise.should.eventually.have.same.members(["error"]).notify(done)
    })

    it("should allow other pieces to capture checking piece", done => {
      const promise = factory.build("board-king-in-check").then(board => moveSequence(board, [["a1", "b1"]]))
      promise.should.eventually.have.same.members(["error"]).notify(done)
    })
  })
  ...
  describe("Ohter pieces causing check", () => {
    it("Should not allow other pieces moving to cause check", done => {
      const promise = factory.build("board-check-from-own-move").then(board => moveSequence(board, [["c1", "c8"]]))
      promise.should.eventually.have.same.members(["error"]).notify(done)
    })
  })
  ...
  describe("Enpassant", () => {
    it("should be able to check with enpassant", done => {
      const promise = factory.build("board-check-enpassant").then(board =>
        moveSequence(board, [
          ["a7", "a5"],
          ["b5", "a6"],
        ])
    ...
    it("should not allow enpassant if it causes check to current player", done => {
      factory.build("board-check-enpassant-suicide").then(board => {
        const result = moveSequence(board, [
          ["c7", "c5"],
          ["b5", "c6"],
        ])
  ...
  describe("Castling causes check", () => {
    it("should be able to check with castling", done => {
      const promise = factory.build("board-check-castling").then(board => moveSequence(board, [["e1", "c1"]]))
  ...
  describe("Two kings", () => {
    it("should not be able to move in range of another king", done => {
      const promise = factory.build("board-check-two-kings").then(board => moveSequence(board, [["a8", "a7"]]))
      promise.should.eventually.have.same.members(["error"]).notify(done)
    })
  })
})
