...
describe("Castling", () => {
  ...
  describe("Under attack", () => {
    ...
    it("should not allow castling if king is in check", () => {
      const results = moveSequence(subject, [["a2", "e2"], ["e1", "c1"]])
      results.should.eql(["move|check", "error"])
    });

    it("should not allow king to move through square that are under attack", () => {
      const results = moveSequence(subject, [["a2", "c2"], ["e1", "c1"]])
      results.should.eql(["move", "error"])
    });

    it("should not allow castling if destination is under attack", () => {
      const results = moveSequence(subject, [["a2", "d2"], ["e1", "c1"]])
      results.should.to.eql(["move", "error"])
    })
