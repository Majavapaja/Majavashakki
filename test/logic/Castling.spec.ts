...
describe("Castling", () => {
  describe("Basics", () => {
    it("should allow castling with left rook", done => {
      const promise = factory.build("board-castling").then(board => moveSequence(board, [["e1", "c1"]]))
      promise.should.eventually.have.same.members(["castling"]).notify(done)
    })

    it("should allow castling with right rook", done => {
      const promise = factory.build("board-castling").then(board => moveSequence(board, [["e1", "g1"]]))
      promise.should.eventually.have.same.members(["castling"]).notify(done)
    })

    it("should not allow castling if king has moved", done => {
      const promise = factory.build("board-castling").then(board =>
        moveSequence(board, [
          ["e1", "f1"],
          ["f1", "e1"],
          ["e1", "c1"],
  ...
  describe("Under attack", () => {
    it("should not allow castling if king is in check", done => {
      const promise = factory.build("board-castling-threat").then(board =>
        moveSequence(board, [
          ["a2", "e2"],
          ["e1", "c1"],
        ])
    ...
    it("should allow castling if rook is threatened", done => {
      const promise = factory.build("board-castling-threat").then(board => moveSequence(board, [["e1", "c1"]]))
      promise.should.eventually.have.same.members(["castling"]).notify(done)
    })
  })
})
